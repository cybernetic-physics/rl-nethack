...
HF_REPO_DIR="${HF_REPO_DIR:-$ROOT_DIR/hf-models}"
HF_SUBDIR="${HF_SUBDIR:-artifacts}"
COMMIT_MSG="${COMMIT_MSG:-sync model artifacts from main repo}"

if [[ ! -d "$HF_REPO_DIR/.git" ]]; then
  echo "HF model repo dir not found: $HF_REPO_DIR" >&2
  exit 1
fi

if [[ $# -eq 0 ]]; then
  echo "Usage: $0 <path> [path ...]" >&2
  echo "Example: $0 output/qwen14b_nld_long_32k_run" >&2
  exit 1
fi
...
git -C "$HF_REPO_DIR" lfs track "*.json" "*.jsonl" "*.safetensors" "*.bin" "*.pt" "*.pth" "*.db" >/dev/null

if [[ ! -f "$HF_REPO_DIR/README.md" ]]; then
  cat > "$HF_REPO_DIR/README.md" <<'EOF'
# rl-nethack-models

Model artifacts and checkpoints for the `rl-nethack` project.
